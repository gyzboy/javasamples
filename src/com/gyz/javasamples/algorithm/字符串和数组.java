...
//        System.out.println(findMost("aabbccc"));
        ...
        int[] arr = new int[]{1, 4, 2, 5, 3, 7, 11, 3, 4};
//        myHeapSort(arr);
//        quickSort(arr, 0, arr.length - 1);
    ...
    public static void heapifySmall(int[] arr, int pos, int len) {
        int small = pos;
        int lson = pos * 2 + 1;
        int rson = pos * 2 + 2;
        if (lson < len && arr[small] > arr[lson]) {
            small = lson;
        }
        if (rson < len && arr[small] > arr[rson]) {
            small = rson;
        }
        if (small != pos) {
            int temp = arr[small];
            arr[small] = arr[pos];
            arr[pos] = temp;
            heapify(arr, small, len);
        }
    }

    public static void quickSort(int[] arr, int low, int high) {
        // 分治的思想，找到中间点，然后分别处理其左右
        if (low < high) {
    ...
    public static int findMid(int[] arr, int low, int high) {
        // 将比pivot小的数，排到左侧
        int pivot = arr[high];
        int i = low;
    ...
    public static void shellSort(int[] arr) {
        ...
        for (int step = len / 2; step > 0; step /= 2) {
            // 从第二个数开始找，因为第一个数已经有序了
            for (int i = step; i < len; i++) {
                int val = arr[i];
                int j;
                for (j = i - step; j >= 0 && arr[j] > val; j -= step) {
                    arr[j + step] = arr[j];
                }
                arr[j + step] = val;
            }
